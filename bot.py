...
<body>
  ...
  <script>
    ...
    async function refresh() {
      ...
      let firstToken = null;
      Object.entries(data.tokens).forEach(([token, d]) => {
      ...
      const selectedToken = ruleSelect.value || firstToken;
      if (selectedToken && data.tokens[selectedToken]) {
        ruleSelect.value = selectedToken;
        fillRuleForm(selectedToken, data.tokens[selectedToken]);
      }
      ...
      (data.followers || []).forEach(f => {
        ...
        `;
        followersMobile.appendChild(card);
      });

      const followHistory = document.getElementById("followHistory");
      followHistory.innerHTML = (data.follow_history || []).slice(-10).reverse().map(h => {
      ...
      if (eventsToggle && !eventsToggle.dataset.bound) {
        eventsToggle.dataset.bound = "1";
        eventsToggle.onclick = () => {
          const ev = document.getElementById("events");
          const show = ev.style.display === "none";
    ...
    walletAddrView.textContent = data.wallet.address || "-";
    const walletBnb = document.getElementById("walletBnb");
    walletBnb.textContent = data.wallet.bnb_balance === null ? "-" : Number(data.wallet.bnb_balance).toFixed(6);
    const walletBnbTs = document.getElementById("walletBnbTs");
    walletBnbTs.textContent = data.wallet_balance_ts ? fmtTs(data.wallet_balance_ts * 1000) : "-";
